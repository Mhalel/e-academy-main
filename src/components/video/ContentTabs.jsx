	...
	return (
		<motion.div className="fixed right-0 bottom-0 z-20 flex flex-col w-full lg:w-1/4 rounded-tl-xl rounded-tr-xl shadow-[#000] shadow-xl ">
			...
			<motion.div
				...
				className={`bg-[#222] flex flex-col justify-start items-start gap-4 ${showLinks || showContent ? "p-5" : "p-0"}`}
				style={{
					overflowY: "auto",
					scrollbarWidth: "none",
			...
			>
				...
				{showContent &&
					dataToShow.map((section) => (
						<>
							<a href={"#"+section.title.replace(" ","-")} key={section.id} className="group flex justify-start items-center gap-5 text-white">
								<h1 className="font-bold font-inter">
									{section.title}
								</h1>
								<span onClick={() => sectionClick(section.start)} className="text-xs font-medium hidden group-hover:flex justify-center items-center gap-2 hover:text-gray-100 px-2 py-0.5 border border-gray-400 rounded-2xl hover-border-gray-100">
									<FaPlay />
